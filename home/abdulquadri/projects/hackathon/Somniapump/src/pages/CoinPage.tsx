...
export default function CoinPage({ factory, router }: Props) {
  ...
  }, [router, tokenAddr, wethAddr, decimals, isPending, slippage]);
    ...
    const quote = async () => {
      ...
      try {
        ...
        const outRaw = Array.isArray(out)
          ...
          : 0n;
        // Apply slippage to show minimum expected output
        const minOut = (outRaw * BigInt(Math.floor((100 - slippage) * 100))) / 10000n;
        setBuyOut(formatUnits(minOut, decimals));
      } catch {}
    };
    quote();
    return () => {
  ...
  }, [router, wethAddr, tokenAddr, buyEth, decimals, slippage]);
  ...
  useEffect(() => {
    ...
    const quote = async () => {
      ...
      try {
        ...
        // Apply slippage to show minimum expected output
        const minOut = (outRaw * BigInt(Math.floor((100 - slippage) * 100))) / 10000n;
        setSellOut(formatUnits(minOut, 18));
      } catch {}
    };
    quote();
    return () => {
  ...
  }, [router, wethAddr, tokenAddr, sellToken, decimals, slippage]);
  ...
  const onBuy = async () => {
    ...
    try {
      ...
      // Refresh price after transaction
      setTimeout(() => {
        // This will trigger the price refresh effect
        setBuyEth(buyEth); // Just to trigger the effect
      }, 1000);
    } catch (e: any) {
  ...
  const onSell = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        // This will trigger the price refresh effect
        setSellToken(sellToken); // Just to trigger the effect
      }, 1000);
    } catch (e: any) {
